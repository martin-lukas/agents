---
name: gsd-init
description: Initialize .planning/ files for a project by assessing the repository and asking key questions.
---
...
- Ask follow-ups one at a time, not as a new batch. Use the `ask_user` tool.

Once answers are solid, read `~/.gemini/templates/planning/` for structure, create `.planning/`, populate the requested files with all known information (no unfilled placeholders), and tell the user what was created.
